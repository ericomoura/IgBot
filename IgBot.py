...
class IgBot:
	...
	#Gets the names of 'target''s followers and returns them in a list
	def get_followers(self, target):
		if(target == ""):	#Target is self
			target = self.get_own_username()

		print("Getting " + target + "'s followers...")
		self.driver.get("https://www.instagram.com/" + target)

		followers_button = self.driver.find_element_by_xpath("//a[text() = ' followers']")
		followers_button.click()
		time.sleep(1)   

		followers_list = self.driver.find_element_by_xpath("//div/ul/div")
		followers_list.click()	#Selects followers list in order to scroll

		num_followers = int(self.driver.find_element_by_xpath("//a[text() = ' followers']/span").text)
		
		#Loads all followers. List starts with 24 profiles and loads 12 more each time.
		list_of_followers = self.driver.find_elements_by_xpath("//div/ul/div/li")
		while(len(list_of_followers) != num_followers):
			self.driver.find_element_by_xpath("//body").send_keys(Keys.END)
			list_of_followers = self.driver.find_elements_by_xpath("//div/ul/div/li")
			time.sleep(0.5)

		#Extracts profile names from the list
		followers = self.driver.find_elements_by_xpath("//a[contains(@class, 'notranslate')]")
		follower_names = []
		for follower in followers:
			follower_names.append(follower.get_attribute("title"))
